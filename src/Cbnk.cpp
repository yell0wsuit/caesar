...
void PrintAllDetails(const vector<CbnkInst> &instruments, const vector<CbnkCwav> &samples, const string &fileName)
{
...
bool Cbnk::Convert(string cwarPath)
{
    ...
    if (!Common::Assert(pos, 0xFEFF, ReadFixLen(pos, 2)))
    {
        return false;
    }
    if (!Common::Assert(pos, 0x20, ReadFixLen(pos, 2)))
    {
        return false;
    }
    ...
    if (!Common::Assert<uint64_t>(pos, Length, ReadFixLen(pos, 4)))
    {
        return false;
    }
    if (!Common::Assert(pos, 0x1, ReadFixLen(pos, 4)))
    {
        return false;
    }
    if (!Common::Assert(pos, 0x5800, ReadFixLen(pos, 4)))
    {
        return false;
    }
    ...
    if (!Common::Assert(pos, 0x494E464F, ReadFixLen(pos, 4, false)))
    {
        return false;
    }
    if (!Common::Assert<uint32_t>(pos, infoLength, ReadFixLen(pos, 4)))
    {
        return false;
    }
    if (!Common::Assert(pos, 0x100, ReadFixLen(pos, 4)))
    {
        return false;
    }
    ...
    if (!Common::Assert(pos, 0x101, ReadFixLen(pos, 4)))
    {
        return false;
    }
    ...
    for (uint32_t i = 0; i < cwavCount; ++i)
    {
        ...
        if (cwav.Id < 0xF000)
        {
            ...
            if (!Common::Assert(pos, 0x52494646, ReadFixLen(pos, 4, false)))
            {
                return false;
            }
            if (!Common::Assert<uint64_t>(pos, cwavLength - 8, ReadFixLen(pos, 4)))
            {
                return false;
            }
            if (!Common::Assert(pos, 0x57415645, ReadFixLen(pos, 4, false)))
            {
                return false;
            }
            if (!Common::Assert(pos, 0x666D7420, ReadFixLen(pos, 4, false)))
            {
                return false;
            }
            if (!Common::Assert(pos, 0x10, ReadFixLen(pos, 4)))
            {
                return false;
            }
            if (!Common::Assert(pos, 0x1, ReadFixLen(pos, 2)))
            {
                return false;
            }
            ...
            if (!Common::Assert(pos - 8, byteRate, (cwav.SampleRate * cwav.ChanCount) * (bitsPerSample / 8)))
            {
                return false;
            }
            if (!Common::Assert<uint16_t>(pos - 4, blockAlign, cwav.ChanCount * (bitsPerSample / 8)))
            {
                return false;
            }
            if (!Common::Assert(pos, 0x64617461, ReadFixLen(pos, 4, false)))
            {
                return false;
            }
    ...
    for (uint32_t i = 0; i < instCount; ++i)
    {
        ...
        if (!Common::Assert(pos, 0x8, ReadFixLen(pos, 4)))
        {
            return false;
        }
        ...
        case 0x6001:
        {
            ...
            if (padding)
            {
                if (!Common::Assert(pos, 0x0, ReadFixLen(pos, 4 - padding)))
                {
                    return false;
                }
            }
        ...
        case 0x6002:
        {
            ...
            if (!Common::Assert(pos, 0x0, ReadFixLen(pos, 2)))
            {
                return false;
            }
        ...
        for (uint32_t j = 0; j < insts[i].NoteCount; ++j)
        {
            ...
            if (!Common::Assert(pos, 0x0, ReadFixLen(pos, 1)))
            {
                return false;
            }
            ...
            if (!Common::Assert(pos, 0x0, ReadFixLen(pos, 3)))
            {
                return false;
            }
        }
    ...
    string outputFileName = FileName.substr(0, FileName.length() - 5) + ".txt";

    // Call the function to print all details
    PrintAllDetails(insts, cwavs, outputFileName);
